...
import com.devteria.post.repository.httpcilent.ProfileClient;
...
public class PostService {
    ...
    public PageResponse<PostResponse> getMyPost(int page, int size){
        ...
        try {
            userProfile = profileClient.getProfile(id).getResult();
        }catch (Exception e){
            log.error("Error with getting user profile ",e);
        }
        ...
        var pageData = postRepository.findAllByUserId(id,pageable);

        String username = userProfile != null ? userProfile.getUserName() : null;
        var postList = pageData.getContent().stream().map(post -> {
            var postResponse = postMapper.toPostResponse(post);
            postResponse.setCreated(dayTimeFormatter.format(postResponse.getCreatedDate()));
